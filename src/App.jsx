...
import {
  ...
  savePortalImageLocally,
  uploadPortalImage,
  uploadPortalImageToCloud
...
const QuoteDocument = React.forwardRef(function QuoteDocument(
  { generatedQuote, className = '' },
  ref
) {
  ...
  return (
    <div
      ref={ref}
      className={`space-y-6 text-xs text-slate-600 font-semibold bg-white ${className}`.trim()}
    >
      <div data-pdf-section="quote-header" className="flex justify-between items-start border-b-2 border-slate-900 pb-6">
        <div>
      ...
      <div data-pdf-section="buyer-info" className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 font-semibold leading-relaxed">
        <div>
      ...
      <div data-pdf-section="request-specs" className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
              ...
              {generatedQuote.localImages && generatedQuote.localImages.length > 0 ? (
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-3">
                    {generatedQuote.localImages.map((imgSrc, idx) => (
                      <div key={idx} className="relative aspect-square border rounded-lg overflow-hidden bg-slate-50 shadow-sm max-h-32 max-w-[120px]">
                        <img src={imgSrc} alt={`견적서첨부이미지-${idx}`} className="w-full h-full object-cover" loading="eager" />
                  ...
                  {generatedQuote.imageUrl && (
                    <div className="overflow-hidden space-y-1">
                      <span className="text-[10px] text-slate-400 block font-bold">원본 이미지 링크</span>
                      <a
                        href={generatedQuote.imageUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="text-indigo-600 hover:underline break-all whitespace-normal block text-[11px] leading-relaxed font-bold"
                      >
                        {generatedQuote.imageUrl}
                      </a>
                    </div>
                ...
                <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-lg border max-w-md">
                  <div className="w-14 h-14 bg-slate-100 rounded-lg overflow-hidden shrink-0 border">
                    <img src={generatedQuote.imageUrl} alt="참조 링크 이미지" className="w-full h-full object-cover" loading="eager" />
                  </div>
                  <div className="overflow-hidden space-y-1">
                    <span className="text-[10px] text-slate-400 block font-bold">인터넷 쇼핑몰 이미지 링크</span>
                    <a href={generatedQuote.imageUrl} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline break-all whitespace-normal block text-[11px] leading-relaxed font-bold">
                      {generatedQuote.imageUrl}
                    </a>
                  </div>
                </div>
      ...
      <div data-pdf-section="pricing-breakdown" className="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100 space-y-2.5">
      ...
      <div data-pdf-section="pricing-table" className="border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div data-pdf-section="payment-terms" className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2.5">
      ...
      <div className="space-y-4">
        <div data-pdf-section="logistics-notice" className="w-full p-3 bg-amber-50 rounded-xl border border-amber-200 text-[11px] text-amber-900 leading-normal font-bold">
        ...
        <div data-pdf-section="total-summary" className="w-full bg-slate-900 text-white p-4 rounded-xl space-y-2 font-semibold">
      ...
      <div data-pdf-section="legal-notice" className="bg-amber-50 p-4 rounded-xl border border-amber-200 text-[11px] text-amber-900 space-y-1.5 font-semibold leading-relaxed">
...
export default function App() {
  ...
  const [imageInputMode, setImageInputMode] = useState(IMAGE_INPUT_MODES.localUpload);
  ...
  const quotePdfExportRef = useRef(null);
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      const imageUrls = await Promise.all(
        files.map((file) => {
          if (imageInputMode === IMAGE_INPUT_MODES.localUpload) {
            return savePortalImageLocally(file);
          }
          if (imageInputMode === IMAGE_INPUT_MODES.cloudUpload) {
            return uploadPortalImageToCloud(file);
          }
      ...
      showNotification(
        imageInputMode === IMAGE_INPUT_MODES.cloudUpload
          ? `${files.length}장의 사진이 클라우드 업로드로 등록되었습니다.`
          : `${files.length}장의 사진이 로컬 기록용으로 등록되었습니다.`
      );
    } catch (error) {
      console.warn('Image upload failed.', error);
      showNotification(
        imageInputMode === IMAGE_INPUT_MODES.cloudUpload
          ? '클라우드 업로드에 실패했습니다. Firebase Storage 상태를 확인해 주세요.'
          : '사진 저장 중 오류가 발생했습니다.',
        'error'
      );
    } finally {
  ...
  const switchImageInputMode = (nextMode) => {
    setImageInputMode(nextMode);
    setUploadedImages([]);
    setProductForm((prev) => ({
      ...prev,
      imageUrl: nextMode === IMAGE_INPUT_MODES.externalUrl ? prev.imageUrl : ''
    }));
  };
  ...
  const handleRequestProduct = async (e) => {
    ...
    if (imageInputMode === IMAGE_INPUT_MODES.externalUrl && (!productForm.imageUrl || !productForm.imageUrl.trim())) {
      showNotification('외부 이미지 링크를 입력해 주세요.', 'error');
      return;
    }
    if (imageInputMode !== IMAGE_INPUT_MODES.externalUrl && uploadedImages.length === 0) {
      showNotification('참고 제품 사진을 1장 이상 필수로 직접 첨부해 주세요.', 'error');
      return;
    }

    let normalizedExternalImageUrl = '';
    let normalizedReferenceImages = [...uploadedImages];

    if (imageInputMode === IMAGE_INPUT_MODES.externalUrl) {
      normalizedExternalImageUrl = productForm.imageUrl.trim();
      const embeddedExternalImage = await fetchRemoteImageAsDataUrl(normalizedExternalImageUrl);

      if (!embeddedExternalImage) {
        showNotification('외부 이미지 링크에서 실제 이미지를 읽지 못했습니다. 직접 이미지 주소를 사용하거나 로컬/Firebase 업로드를 이용해 주세요.', 'error');
        return;
      }

      normalizedReferenceImages = [embeddedExternalImage];
    }
    ...
    const newRequest = {
      ...
      imageUrl: imageInputMode === IMAGE_INPUT_MODES.externalUrl ? normalizedExternalImageUrl : '',
      localImages: normalizedReferenceImages
    };
    ...
    setImageInputMode(IMAGE_INPUT_MODES.localUpload);
    setUploadedImages([]); 
...
※ 자세한 사항은 안내드린 위챗/카카오톡을 통해 전문 컨설팅과 정식 인보이스를 참고바랍니다.`;
  ...
  const handleDownloadQuotePdf = async () => {
    const sourceElement = quotePdfExportRef.current || quotePdfRef.current;
    if (!generatedQuote || !sourceElement || isDownloadingPdf) return;

    setIsDownloadingPdf(true);
    const originalImageSources = [];

    try {
      const exportImages = Array.from(sourceElement.querySelectorAll('img'));
      await Promise.all(
        exportImages.map(async (image) => {
          const imageSource = image.getAttribute('src') || '';

          if (imageSource && !imageSource.startsWith('data:')) {
            const embeddedImageSource = await fetchRemoteImageAsDataUrl(imageSource);
            if (embeddedImageSource) {
              originalImageSources.push({ image, imageSource });
              image.setAttribute('src', embeddedImageSource);
            }
          }
      ...
      const canvasScaleY = canvas.height / sourceElement.scrollHeight;
      const sectionBoundaries = Array.from(sourceElement.querySelectorAll('[data-pdf-section]'))
        .map((section) => {
          const sectionRect = section.getBoundingClientRect();
          const sectionStart = sectionRect.top - sourceRect.top + sourceElement.scrollTop;
          const sectionEnd = sectionRect.bottom - sourceRect.top + sourceElement.scrollTop;

          return {
            start: Math.round(sectionStart * canvasScaleY),
            end: Math.round(sectionEnd * canvasScaleY)
          };
        })
        .filter((section) => section.end > section.start);

      const longImageData = canvas.toDataURL('image/png', 1);
      const longImage = new Image();
      await new Promise((resolve, reject) => {
        longImage.onload = resolve;
        longImage.onerror = reject;
        longImage.src = longImageData;
      });
      ...
      const getSliceHeight = (startHeight) => {
        const remainingHeight = canvas.height - startHeight;
        const idealSliceHeight = Math.min(pageCanvasHeight, remainingHeight);

        if (idealSliceHeight <= 0) return 0;

        const proposedEnd = startHeight + idealSliceHeight;
        const overlappingSection = sectionBoundaries.find(
          (section) =>
            section.start > startHeight &&
            section.start < proposedEnd &&
            section.end > proposedEnd &&
            section.end - section.start < pageCanvasHeight
        );
        ...
        const adjustedSliceHeight = overlappingSection.start - startHeight;
        if (adjustedSliceHeight >= MIN_PAGE_SLICE_HEIGHT) {
          return adjustedSliceHeight;
        }
      ...
      while (renderedHeight < canvas.height) {
        const sliceHeight = getSliceHeight(renderedHeight);
        if (sliceHeight <= 0) {
          break;
        }
      ...
      originalImageSources.forEach(({ image, imageSource }) => {
              ...
              <form onSubmit={handleRequestProduct} className="space-y-5">
                ...
                <div className="border border-slate-200 p-4 rounded-xl bg-slate-50 space-y-4">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-2 bg-white p-1 rounded-lg border text-xs">
                      <button
                        type="button"
                        onClick={() => switchImageInputMode(IMAGE_INPUT_MODES.localUpload)}
                        className={`px-3 py-1.5 rounded-md font-bold transition-all ${imageInputMode === IMAGE_INPUT_MODES.localUpload ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-800'}`}
                      >
                        본체 업로드 기록
                      </button>
                      <button
                        type="button"
                        onClick={() => switchImageInputMode(IMAGE_INPUT_MODES.cloudUpload)}
                        className={`px-3 py-1.5 rounded-md font-bold transition-all ${imageInputMode === IMAGE_INPUT_MODES.cloudUpload ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-800'}`}
                      >
                        Firebase 업로드
                      </button>
                      <button
                        type="button"
                        onClick={() => switchImageInputMode(IMAGE_INPUT_MODES.externalUrl)}
                        className={`px-3 py-1.5 rounded-md font-bold transition-all ${imageInputMode === IMAGE_INPUT_MODES.externalUrl ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-800'}`}
                      >
                        외부 이미지 URL 입력
                      </button>
                    </div>
                  ...
                  {imageInputMode === IMAGE_INPUT_MODES.externalUrl ? (
                    <div>
                      <label className="block text-xs font-extrabold text-slate-700 mb-1">외부 이미지 주소 <span className="text-red-500 font-black">* 필수</span></label>
                      <input 
                        type="url" 
                        required={imageInputMode === IMAGE_INPUT_MODES.externalUrl}
                        placeholder="https://... 형태의 직접 열리는 이미지 주소"
                    ...
                    <div className="space-y-3">
                      <label className="block text-xs font-extrabold text-slate-700 mb-1">
                        {imageInputMode === IMAGE_INPUT_MODES.localUpload ? '제품 참고 실물 사진 파일 등록' : 'Firebase Storage 업로드용 사진 등록'}
                        <span className="text-red-500 font-black"> * 필수 (3장 이내)</span>
                      </label>
                      <p className="text-[11px] text-slate-500 font-semibold">
                        {imageInputMode === IMAGE_INPUT_MODES.localUpload
                          ? '기본값입니다. 사진을 압축한 뒤 데이터와 함께 저장하므로 가장 안정적으로 기록됩니다.'
                          : 'Firebase Storage가 정상 연결된 경우 공개 이미지 주소로 저장됩니다.'}
                      </p>
                          ...
                          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2.5">
                            ...
                            {selectedRequest.localImages && selectedRequest.localImages.length > 0 ? (
                              <div className="space-y-2">
                                <div className="grid grid-cols-3 gap-3">
                                  {selectedRequest.localImages.map((src, idx) => (
                                    <div key={idx} className="relative aspect-square border rounded-lg overflow-hidden bg-white shadow-sm">
                                      <img src={src} alt="바이어제공_실물사진" className="w-full h-full object-cover" loading="eager" />
                                ...
                                {selectedRequest.imageUrl && (
                                  <div className="overflow-hidden space-y-1 font-semibold">
                                    ...
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noreferrer"
                                      className="text-indigo-600 text-xs hover:underline break-all whitespace-normal block leading-relaxed max-w-md"
                                    >
                                      {selectedRequest.imageUrl}
                                    </a>
                                  </div>
                              ...
                              <div className="flex items-center gap-3 bg-white p-2.5 rounded-lg border">
                                <div className="w-14 h-14 bg-slate-100 rounded-lg overflow-hidden shrink-0 border">
                                  <img src={selectedRequest.imageUrl} alt="레퍼런스 이미지" className="w-full h-full object-cover" loading="eager" />
                                </div>
                                <div className="overflow-hidden space-y-1 font-semibold">
                                  <span className="text-[10px] text-slate-400 block font-bold">참조용 네이버/쿠팡 판매 URL</span>
                                  <a href={selectedRequest.imageUrl} target="_blank" rel="noreferrer" className="text-indigo-600 text-xs hover:underline break-all whitespace-normal block leading-relaxed max-w-md">
                                    {selectedRequest.imageUrl}
                                  </a>
                                </div>
                              </div>
      ...
      {showQuoteModal && generatedQuote && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
            ...
            <div className="absolute left-[-10000px] top-0 pointer-events-none overflow-hidden bg-white" aria-hidden="true">
              <QuoteDocument
                ref={quotePdfExportRef}
                generatedQuote={generatedQuote}
                className="w-[860px] p-8"
              />
            </div>
