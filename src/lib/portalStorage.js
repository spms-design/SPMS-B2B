...
const loadRemoteImageElement = (sourceUrl) =>
  new Promise((resolve, reject) => {
    const image = new Image();
...
export const savePortalImageLocally = async (file) => fileToOptimizedDataUrl(file);

export const fetchRemoteImageAsDataUrl = async (sourceUrl) => {
  ...
  try {
    ...
    if (!imageBlob.type.startsWith('image/')) {
      throw new Error(`Unsupported content type: ${imageBlob.type || 'unknown'}`);
    }

    const dataUrl = await blobToDataUrl(imageBlob);
    const imageFile = new File([imageBlob], 'reference-image', {
      type: imageBlob.type || 'image/jpeg'
    });

    return (await fileToOptimizedDataUrl(imageFile)) || dataUrl;
  } catch (fetchError) {
    try {
      const image = await loadRemoteImageElement(normalizedUrl);
      const maxDimension = 1280;
      const scale = Math.min(1, maxDimension / Math.max(image.width, image.height));
      const canvas = document.createElement('canvas');
      ...
      console.warn('Unable to convert remote image into embeddable data URL.', {
        normalizedUrl,
        fetchError,
        imageError
      });
...
export const uploadPortalImageToCloud = async (file) => {
